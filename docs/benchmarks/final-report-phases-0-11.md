...
Date: 2026-06-18 (Phase 9 added 2026-06-19; Phase 10 added 2026-06-20; Phase 11 added 2026-06-21; Phase 6 re-enabled and run 2026-06-21)
Repository: `proto-test` (`im.chat.v1` protobuf schema, C++20, CMake + FetchContent, Google Benchmark v1.9.1, protobuf v35.1)

This report consolidates every benchmark phase run in this project: infrastructure, throughput/latency/size, field-fill-rate, numeric encoding, scalability, memory/Arena allocation, serialization API overhead, CPU microarchitecture counters, concurrency scaling, malformed-input parse cost, a Protobuf-vs-SBE comparison, a JSON-library shootout, and a yyjson-vs-Protobuf PK. Phase 6 (CPU microarchitecture counters via `perf`) was initially found infeasible in this sandbox (documented as skipped) and was later re-enabled and run for real once the host's `perf_event_paranoid` setting was lowered (2026-06-21).
...
## Phase 6 — CPU Microarchitecture Metrics

Originally skipped: `perf stat` requires `perf_event_paranoid <= 2`, and this host had it set to `4` with no passwordless `sudo` available in that unattended session (documented in `docs/benchmarks/phase6-feasibility-note.md`). The user later lowered `perf_event_paranoid` to `2` on the host, unblocking it; the phase was re-run for real on 2026-06-21. Full methodology (including a hybrid P-core/E-core pinning adjustment the original design didn't anticipate) and results are in `docs/benchmarks/phase6-cpu-microarch-analysis.md`.

`perf stat -e cycles,instructions,cache-misses,branch-misses`, pinned to the host's P-cores (`taskset -c 0-11`) to avoid the hybrid CPU's `cpu_atom`/`cpu_core` PMU split, was used to correlate two prior findings with hardware counters:

- **Phase 5's `CodedOutputStream` slowdown is a pure extra-instructions effect, not a stall.** Versus the fastest variant (`SerializeToPreallocatedArray`), `CodedOutputStream` costs +21% cycles/iter and +23% instructions/iter, while IPC stays flat (3.83-3.90) across all three serialize-API variants tested and cache-miss/branch-miss rates stay in the same sub-0.001-per-iteration noise band for all of them. The virtual-dispatch overhead Phase 5 inferred from API knowledge shows up as more retired instructions per call, not as more pipeline stalls.
- **Phase 3's repeated-message slowdown is disproportionately a cache-miss effect, not just an instruction-count effect.** Per-element marginal cost for `repeated message` (`merged_forward.items`) vs packed `repeated int64` (`mentioned_user_ids`): 23.2x more cycles, 16.4x more instructions, **81.9x more cache misses**, but only 1.4x more branch mispredictions. Cache misses scale up ~5x more steeply than instructions do — the dominant microarchitectural signature of the repeated-message cost is memory-access scatter (each `ForwardedItem` sub-message touching new, less-contiguous memory), not branch misprediction.

Absolute cache-miss/branch-miss counts are small (sub-1-per-iteration for most targets) and noisier (3.6%-16% relative stddev) than cycles/instructions (<1.3%) — read the ratios above as directional, not exact.
...
7. **Phase 6 (CPU microarchitecture counters) confirms *why* two earlier findings are slow, at the hardware level, and the two reasons are different in kind.** Phase 5's `CodedOutputStream` slowdown is a pure extra-instructions effect (+21-23% cycles/instructions vs the fastest variant, flat IPC, no cache/branch signal) — it does more work per call, it doesn't stall more. Phase 3's repeated-message-vs-packed-scalar slowdown is disproportionately a cache-miss effect (cache misses scale ~82x per element vs ~16x for instructions and ~1.4x for branch mispredictions) — it's memory-access scatter, not branch misprediction, that explains the gap beyond raw instruction growth. (Originally this phase could not run due to `perf_event_paranoid=4`; it was re-enabled once that was lowered on the host.)
...
| 6 | — | — | `results/phase6-2026-06-21.txt` | `docs/benchmarks/phase6-cpu-microarch-analysis.md` (originally skipped — see `docs/benchmarks/phase6-feasibility-note.md` for the historical infeasibility finding) |
