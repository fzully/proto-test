...
日期：2026-06-18（Phase 9 于 2026-06-19 补充；Phase 10 于 2026-06-20 补充；Phase 11 于 2026-06-21 补充；Phase 6 于 2026-06-21 重新启用并实测）
...
本报告汇总了本项目中跑过的全部 benchmark 阶段：基础设施、吞吐量/延迟/体积、字段填充率、数值编码、可扩展性、内存/Arena 分配、序列化 API 开销、CPU 微架构计数器、并发扩展性、畸形输入解析开销、一次 Protobuf 与 SBE 的对比、一次 JSON 库的横向对比，以及一次 yyjson 与 Protobuf 的对决。Phase 6（通过 `perf` 采集 CPU 微架构计数器）最初在沙箱环境下被判定为不可行（记录为"跳过"），后来宿主机的 `perf_event_paranoid` 设置被调低后，已于 2026-06-21 重新启用并实测。
...
最初被记录为跳过：`perf stat` 需要 `perf_event_paranoid <= 2`，而这台机器在无人值守的那次执行中是 `4`，且没有免密 `sudo` 可以调低（详见 `docs/benchmarks/phase6-feasibility-note.md`）。之后用户把宿主机的 `perf_event_paranoid` 调低到了 `2`，解除了这个限制；本阶段已于 2026-06-21 重新实测。完整方法论（包括一个原设计没预料到的、混合 P-core/E-core 架构下的 CPU 绑核调整）和结果见 `docs/benchmarks/phase6-cpu-microarch-analysis.md`。

用 `perf stat -e cycles,instructions,cache-misses,branch-misses`，绑定到宿主机的 P-core（`taskset -c 0-11`，避开混合架构 CPU 的 `cpu_atom`/`cpu_core` PMU 拆分问题），把两个此前的发现拿硬件计数器重新核实了一遍：

- **Phase 5 里 `CodedOutputStream` 变慢，本质是"多执行了指令"，不是"更容易停顿"。** 相比最快的变体（`SerializeToPreallocatedArray`），`CodedOutputStream` 每次调用多花 21% 的 cycles、多执行 23% 的指令，而三个序列化 API 变体测下来 IPC 基本不变（3.83-3.90），cache-miss/branch-miss 速率也都落在同一个噪声量级（每次迭代不到 0.001）里分不出差异。Phase 5 当时基于 API 原理推断出的"虚函数调用与记账开销"，在硬件层面体现为每次调用多执行了若干条指令，而不是流水线停顿更多。
- **Phase 3 里 repeated message 变慢，不成比例地是个 cache-miss 效应，不只是指令数效应。** 按元素归一化后，`repeated message`（`merged_forward.items`）相对 packed `repeated int64`（`mentioned_user_ids`）的边际成本：cycles 高 23.2 倍，指令数高 16.4 倍，**cache miss 高 81.9 倍**，但 branch misprediction 只高 1.4 倍。Cache miss 的增长幅度比指令数陡峭了近 5 倍——repeated message 这部分开销在微架构层面最主要的特征是内存访问的离散度（每个 `ForwardedItem` 子消息构造时都要碰到新的、不连续的内存），而不是分支预测失败。

Cache-miss/branch-miss 这两个计数器的绝对值本身很小（多数 target 里每次迭代不到 1 次），相对标准差也比 cycles/instructions（<1.3%）大得多（3.6%-16%）——上面的倍数应该当作方向性证据来看，不是精确数值。
...
7. **Phase 6（CPU 微架构计数器）从硬件层面证实了另外两个发现"为什么慢"，而且两者的成因性质不同。** Phase 5 里 `CodedOutputStream` 变慢，是纯粹的"多执行指令"效应（相比最快变体多花 21-23% 的 cycles/指令，IPC 不变，没有 cache/branch 信号）——它只是每次调用做了更多事，并没有更容易停顿。Phase 3 里 repeated message 相对 packed 标量的变慢，不成比例地是个 cache-miss 效应（按元素归一化后，cache miss 高出约 82 倍，远超指令数的约 16 倍和 branch misprediction 的约 1.4 倍）——是内存访问的离散度，而不是分支预测失败，解释了超出"纯指令数增长"之外的那部分差距。（本阶段最初因 `perf_event_paranoid=4` 跑不起来，宿主机调低该设置后已重新启用。）
...
| 6 | — | — | `results/phase6-2026-06-21.txt` | `docs/benchmarks/phase6-cpu-microarch-analysis.md`（最初跳过——历史的不可行性记录见 `docs/benchmarks/phase6-feasibility-note.md`） |
